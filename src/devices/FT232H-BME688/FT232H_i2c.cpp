...
DWORD FT232H_I2C::writeByte(UCHAR slaveAddress, UCHAR registerAddress,
                            UCHAR data) {
  ...
  buffer[bytesToTransfer++] =
      registerAddress; // Byte addressed inside EEPROM's memory
  buffer[bytesToTransfer++] = data;
  status = I2C_DeviceWrite(
      ftHandle, slaveAddress, bytesToTransfer, buffer, &bytesTransfered,
...
FT_STATUS FT232H_I2C::readByte(UCHAR slaveAddress, UCHAR registerAddress,
                               UCHAR *data) {
  FT_STATUS status;
  DWORD bytesToTransfer = 0;
  DWORD bytesTransfered;
